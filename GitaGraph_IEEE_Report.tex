...
%% ── Attractive code-listing theme (Dracula / VS Code Dark+) ────────────
\definecolor{codebg}{HTML}{1E1E2E}
\definecolor{codeframe}{HTML}{44475A}
\definecolor{linenumfg}{HTML}{636DA6}
\definecolor{kwcolor}{HTML}{BD93F9}
\definecolor{strcolor}{HTML}{F1FA8C}
\definecolor{cmtcolor}{HTML}{6272A4}
\definecolor{numcolor}{HTML}{FFB86C}
\definecolor{fncolor}{HTML}{50FA7B}
\definecolor{typecolor}{HTML}{8BE9FD}
...
\lstdefinestyle{dracula}{
  ...
  framesep=5pt,
  numbers=left,
  numberstyle=\ttfamily\tiny\color{linenumfg},
  numbersep=10pt,
  xleftmargin=22pt,
  xrightmargin=4pt,
  keywordstyle=\bfseries\color{kwcolor},
  keywordstyle={[2]\color{typecolor}},
  keywordstyle={[3]\color{fncolor}},
...
\lstdefinelanguage{Python3}[]{Python}{
  morekeywords={[2]True,False,None,self,cls,str,int,float,dict,list,tuple,set},
  morekeywords={[3]def,class,return,import,from,as,lambda},
}


\lstdefinelanguage{SPARQL}{
  keywords={PREFIX,SELECT,WHERE,FILTER,OPTIONAL,UNION,GRAPH,ASK,CONSTRUCT,DESCRIBE,FROM,NAMED,ORDER,BY,LIMIT,OFFSET,DISTINCT,REDUCED,AS,GROUP,HAVING,VALUES,BIND,SERVICE,MINUS,NOT,EXISTS,IN,STR,LANG,LANGMATCHES,DATATYPE,BOUND,IRI,URI,BNODE,RAND,ABS,CEIL,FLOOR,ROUND,CONCAT,STRLEN,UCASE,LCASE,ENCODE,CONTAINS,STRSTARTS,STRENDS,STRBEFORE,STRAFTER,YEAR,MONTH,DAY,HOURS,MINUTES,SECONDS,TIMEZONE,NOW,UUID,MD5,SHA1,SHA256,SHA384,SHA512,COALESCE,IF,STRLANG,STRDT,SAMETERM,ISIRI,ISURI,ISBLANK,ISLITERAL,ISNUMERIC,COUNT,SUM,MIN,MAX,AVG,SAMPLE,GROUP_CONCAT,true,false},
  morecomment=[l]{\#},
  morestring=[b]",
  morestring=[b]',
}
...
\author{
\IEEEauthorblockN{%
  Hariom Rajput\IEEEauthorrefmark{1},\quad
  Ayushi Choyal\IEEEauthorrefmark{1},\quad
  Ravi Kant Gupta\IEEEauthorrefmark{1},\quad
  Shouryavi Awasthi\IEEEauthorrefmark{1}%
}
\IEEEauthorblockA{%
  \IEEEauthorrefmark{1}Department of Computer Applications,
  National Institute of Technology, Kurukshetra\\
  Kurukshetra, Haryana, India \quad
  \texttt{\{524110006, 524410017, 524410027, 524410028\}@nitkkr.ac.in}%
}
}
...
%-----------------------------------------------------------------------
\begin{abstract}
The Bhagavad G\={i}t\={a}, comprising 700 Sanskrit verses across 18 chapters,
...
verses with mean average precision 0.81; and MYCIN combination reaches
...
Mah\={a}bh\={a}rata. Comprising 700 \'{s}lokas (verses) in 18 chapters
...
\begin{itemize}[leftmargin=*]
  \item \textbf{OWL~2 Ontology:} A formally verified Bhagavad G\={i}t\={a}
        ontology with 16 classes, 9 object properties (including
        \texttt{owl:TransitiveProperty}, \texttt{owl:SymmetricProperty},
        and property chain axioms), encoding 658 RDF triples.

  \item \textbf{Multi-Algorithm Graph Search:} BFS, DFS, A*, and IDDFS
        operating on a 61-node, 175-edge philosophical knowledge graph,
        with an admissible A* heuristic and full UI pathway visualization.

  \item \textbf{Production-Rule Expert System:} An 8-rule forward-chaining
        system with Hindi/Hinglish keyword support, specificity-based conflict
        resolution, and 8 SPARQL Competency Questions (CQs).

  \item \textbf{CSP Study Planner:} Backtracking with MRV heuristic and
        forward checking satisfying 7 hard constraints for personalized verse
        scheduling.

  \item \textbf{Multi-Modal Uncertainty Handler:} MYCIN certainty factors,
        fuzzy yoga-path membership, and non-monotonic belief revision.

  \item \textbf{Semantic RAG Layer:} Sentence-transformer embeddings (all-MiniLM-L6-v2)
        and FAISS indexing over all 701 verses, enabling natural language
        semantic similarity search beyond keyword matching.

  \item \textbf{Ollama LLM Integration:} Local LLM commentary generation
        (Llama~3 via Ollama) producing contextual Sanskrit verse explanations
        in English, Hindi, and Hinglish, with per-message language toggling.

  \item \textbf{React SPA with Ancient Manuscript UI:} An interactive
        single-page application built with React~18, Vite, Tailwind~CSS,
        and Framer Motion, featuring a monarch-period parchment aesthetic
        with illuminated verse cards and a Flask REST API backend with
        16 endpoints.
\end{itemize}
...
Five gaps motivate GitaGraph:
(1)~No OWL~2 ontology with formal axioms for the Bhagavad G\={i}t\={a};
(2)~No system applies multiple graph search algorithms to philosophical
concept navigation;
(3)~No CSP-based personalized study planner for Sanskrit texts;
(4)~No unified uncertainty framework integrating MYCIN, fuzzy logic, and
non-monotonic reasoning;
(5)~No system combines symbolic knowledge-base reasoning with neural
semantic retrieval and local LLM commentary in a multilingual interface.
...
\centering
\small
\begin{tabular}{@{}l r p{3.0cm}@{}}
\toprule
\textbf{Component} & \textbf{Count} & \textbf{Notes} \\
\midrule
Total verses (corpus) & 701 & 18 chapters \\
Annotated verses & 30 & Chapters 2, 3, 6 \\
\quad Ch.~2 (S\={a}\d{n}khya Yoga) & 10 & Self, Ni\d{s}k\={a}ma Karma \\
\quad Ch.~3 (Karma Yoga) & 10 & Action, Gu\d{n}as \\
\quad Ch.~6 (Dhy\={a}na Yoga) & 10 & Meditation, Sam\={a}dhi \\
...
\textbf{Layer} & \textbf{Technology} & \textbf{Version} \\
\midrule
\multicolumn{3}{l}{\textit{Backend (Python)}} & \multicolumn{3}{l}{\textit{Semantic RAG \& LLM Layer}} \\
Ontology       & RDF/OWL 2 (Turtle)       & W3C Rec.       & Embeddings        & sentence-transformers     & 2.7 \\
Semantic eng.  & RDFLib                   & 7.0            & Embedding model   & all-MiniLM-L6-v2          & 384-d \\
Query lang.    & SPARQL 1.1               & W3C Rec.       & Vector index      & FAISS (IndexFlatIP)       & 1.8 \\
Graph alg.     & NetworkX                 & 3.3            & LLM runtime       & Ollama                    & 0.3+ \\
Uncertainty    & Pure Python (CF + Fuzzy) & 3.11+          & LLM model         & Llama 3 (8B, 4-bit quant) & Q4\_K\_M \\
REST API       & Flask                    & 3.0            & Languages         & EN / Hindi / Hinglish     & — \\
Data layer     & Pandas                   & 2.2            & \multicolumn{3}{l}{\textit{Frontend (JavaScript)}} \\
               &                          &                & Framework         & React                     & 18 \\
\multicolumn{3}{l}{}                                     & Build tool        & Vite                      & 5.4 \\
\multicolumn{3}{l}{}                                     & Styling           & Tailwind CSS              & 3.4 \\
\multicolumn{3}{l}{}                                     & State/queries     & TanStack React Query      & 5 \\
\midrule
\multicolumn{6}{l}{\textit{Language: Python~3.11+ / JavaScript ES2022}} \\
\bottomrule
\end{tabular}
\end{table*}
...
\begin{equation}
  ...
  \label{eq:heuristic}
\end{equation}
This heuristic never overestimates: a DownfallCause requires at minimum
4 conceptual steps to reach Moksha.
...
\textit{dhyan}), and Unicode Hindi (\textit{\hi{चिंता}}, \textit{\hi{गुस्सा}},
\textit{\hi{ध्यान}}), enabling native script queries. Rule~R9 (added in v2.1)
maps grief/sorrow keywords (\textit{dukh}, \textit{dard}, \textit{\hi{शोक}}) to
the AtmaJnana concept and Verse~2.20.

\textbf{SPARQL Competency Question (CQ3 — Transitive Chain):}
\begin{figure*}[!t]
\begin{lstlisting}[language=SPARQL, caption={CQ3: Desire downfall chain via transitivity}]
...
\begin{figure*}[!t]
\begin{lstlisting}[language=Python3, caption={Core module interfaces}]
...
GET  & \texttt{/api/sparql/<cq>}     & M4 — SPARQL CQs \\
...
\begin{figure*}[!t]
\begin{lstlisting}[language=Python3, caption={A* search with philosophical heuristic}]
import heapq
...
\begin{figure*}[!t]
\begin{lstlisting}[language=Python3, caption={Semantic search: encode, index, retrieve}]
...
\begin{figure*}[!t]
\begin{lstlisting}[language=Python3, caption={Ollama contextual commentary generation}]
LANG_INSTRUCTION = {
...
def contextualize(verse_ref, english, sanskrit,
    ...
    prompt = (
        ...
        "explaining how this verse addresses the seeker's concern."
    )
    resp = requests.post(
...
\begin{table*}[!t]
\caption{Graph Search Algorithm Performance on the GitaGraph Knowledge Graph}
\label{tab:search}
\centering
\begin{tabular}{@{}l l l c r l@{}}
\toprule
\textbf{Algorithm} & \textbf{Start Node} & \textbf{Goal / Depth} & \textbf{Hops} & \textbf{Time (ms)} & \textbf{Path / Notes} \\
\midrule
BFS   & NishkamaKarma  & 2 hops              & ---  & 1.2  & 7 verses: 2.47, 2.48, 2.71, 3.9, 3.19, 3.3, 3.35 \\
...
A*    & Kama           & Moksha              & 7    & 3.4  & No direct path — guided through conceptual detour \\
IDDFS & Arjuna (Ch.1)  & KarmaYoga ($d$=3)   & 3    & 4.7  & Iterations: $d$=1 (3 nodes), $d$=2 (9), $d$=3 (goal found) \\
\bottomrule
\end{tabular}
\end{table*}
...
\toprule
\textbf{Reader Profile} & \textbf{Rule(s) Fired} & \textbf{Verse Recommendation} & \textbf{CF} \\
\midrule
Anxious beginner          & R3 (specificity=3), R1 & Verse 2.47 — NishkamaKarma (Karma Yoga, Ch.~2)  & 0.92 \\
Anger / desire (Hindi)    & R4, R9                 & Kama downfall chain, Ch.~3                       & 0.90 \\
Meditation seeker         & R5                     & Verse 6.10 — DhyanaYoga (Dhyana Yoga, Ch.~6)    & 0.95 \\
Duty confused             & R6                     & Verse 3.35 — Svadharma (Karma Yoga, Ch.~3)      & 0.88 \\
Grief (``dukh'')          & R9 \textit{(new v2.1)} & Verse 2.20 — AtmaJnana (Jnana Yoga, Ch.~2)      & 0.82 \\
Advanced / liberation     & R8                     & Moksha spiritual progression path               & 0.88 \\
\bottomrule
\end{tabular}
\end{table*}
...
\begin{tabular}{llrr}
\toprule
\textbf{Query} & \textbf{Top-1 Verse} & \textbf{Score} & \textbf{MAP@5} \\
\midrule
...
\centering
\small
\begin{tabular}{@{}l p{3.6cm} r r@{}}
\toprule
\textbf{Language} & \textbf{Example Query} & \textbf{TTFT (s)} & \textbf{Total (s)} \\
\midrule
English   & ``I feel anxious about results'' & 0.9 & 4.2 \\
Hindi     & ``\hi{मुझे कर्म की चिंता है}'' (Mujhe karm ki chinta hai) & 1.1 & 5.1 \\
...
\begin{table*}[!t]
\caption{Feature Comparison with Existing Bhagavad G\={i}t\={a} Digital Systems}
\label{tab:comparison}
\centering
\begin{tabular}{@{}l ccccc@{}}
\toprule
\textbf{Feature} & \textbf{Vedabase.io} & \textbf{BG App} & \textbf{DBpedia} & \textbf{GitaGraph v1} & \textbf{GitaGraph v2.1 (Ours)} \\
\midrule
Keyword search      & \checkmark & \checkmark & \checkmark & \checkmark & \checkmark \\
OWL 2 ontology      & $\times$   & $\times$   & Partial    & \checkmark & \checkmark \\
Graph traversal     & $\times$   & $\times$   & $\times$   & \checkmark & \checkmark \\
Expert system       & $\times$   & $\times$   & $\times$   & \checkmark & \checkmark \\
CSP study planner   & $\times$   & $\times$   & $\times$   & \checkmark & \checkmark \\
...
gives $(0.90 + 0.85 + 0.70)/3 = 0.817$, significantly underestimating
convergent evidence. MYCIN correctly yields 0.9991.
...
\textbf{Limitations:}
\begin{enumerate}[leftmargin=*]
  \item \textbf{Annotation coverage:} Only 30 of 701 verses are annotated in the
        knowledge graph; Chapters~4--18 lack conceptual links. Graph-based modules
...
in \textit{Proc. SciPy 2008}, Pasadena, CA, 2008, pp.~11--15.
...
in \textit{Proc. IJCAI Workshop on Basic Ontological Issues}, 1995.
...
``A linked research network transforming musicology,''
in \textit{Proc. WHiSe at ESWC 2016}, Heraklion, 2016.
...
\label{tab:tasks}
\centering
\begin{tabular}{p{3.0cm}p{2.0cm}p{10.5cm}}
\toprule
...
\begin{figure*}[!t]
\begin{lstlisting}[language=Python3, caption={MYCIN certainty factor combination}]
...
\begin{figure*}[!t]
\begin{lstlisting}[language=Python3, caption={CSP backtracking with MRV + forward checking}]
...
\begin{figure*}[!t]
\begin{lstlisting}[language=Python3, caption={Production rule with multilingual keyword support}]
PRODUCTION_RULES = [
  {
    'id': 'R1_AnxietyStress',
    'condition': lambda wm: any(k in wm.get('concern','').lower()
        for k in ['anxious', 'stress', 'worry', 'anxi',
                  'tension', 'chinta',        # Hinglish
                  'bechained', 'pareshan',
                  # Hindi: 'chinta' (concern)
                  # Hindi: 'tanav' (stress)
...
\begin{figure*}[!t]
\centering
\includegraphics[width=0.85\textwidth]{figures/ontology.png}
