...
Local LLM runtime & Ollama                    & 0.3+ \\
LLM model         & Llama 3 (8B, 4-bit quant) & Q4\_K\_M \\
...
Icons             & Lucide React              & — \\
\midrule
Language          & Python / JavaScript       & 3.11+ / ES2022 \\
\bottomrule
\end{tabular}
\end{table}
...
\textit{dhyan}), and Unicode Hindi (\textit{चिंता}, \textit{गुस्सा},
\textit{ध्यान}), enabling native script queries. Rule~R9 (added in v2.1)
maps grief/sorrow keywords (\textit{dukh}, \textit{dard}, \textit{शोक}) to
...
\begin{lstlisting}[language=Python, caption={Semantic search: encode, index, retrieve}]
...
\begin{lstlisting}[language=Python, caption={Ollama contextual commentary generation}]
LANG_INSTRUCTION = {
...
BFS   & NishkamaKarma  & 2 hops              & --  & 1.2 \\
      & \multicolumn{3}{l}{\small 7 verses: 2.47, 2.48, 2.71, 3.9, 3.19, 3.3, 3.35} \\
\midrule
DFS   & Kama           & BuddhiNasha         & 4   & 0.8 \\
      & \multicolumn{3}{l}{\small Kama $\to$ Krodha $\to$ Moha $\to$ BuddhiNasha} \\
\midrule
A*    & Vairagya       & Moksha              & 3   & 2.1 \\
      & \multicolumn{3}{l}{\small Vairagya $\to$ ChittaShuddhi $\to$ AtmaJnana $\to$ Moksha} \\
A*    & DhyanaYoga     & Moksha              & 3   & 1.9 \\
      & \multicolumn{3}{l}{\small DhyanaYoga $\to$ Samadhi $\to$ AtmaJnana $\to$ Moksha} \\
A*    & Kama           & Moksha              & 7   & 3.4 \\
      & \multicolumn{3}{l}{\small No direct path — guided through detour} \\
\midrule
IDDFS & Arjuna (Ch.1)  & KarmaYoga (d=3)     & 3   & 4.7 \\
      & \multicolumn{3}{l}{\small Iterations: d=1 (3 nodes), d=2 (9), d=3 (found)} \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{tabular}{llll}
\toprule
\textbf{Profile} & \textbf{Rule(s)} & \textbf{Recommendation} & \textbf{CF} \\
\midrule
Anxious beginner       & R3 (sp=3), R1 & Verse 2.47 (NishkamaKarma) & 0.92 \\
Anger/desire (Hindi)   & R4, R9        & Kama chain, Ch.~3          & 0.90 \\
Meditation seeker      & R5            & Verse 6.10 (DhyanaYoga)    & 0.95 \\
Duty confused          & R6            & Verse 3.35 (Svadharma)     & 0.88 \\
Grief (``dukh'')       & R9 (new v2.1) & Verse 2.20 (AtmaJnana)     & 0.82 \\
...
\centering
\small
\begin{tabular}{p{2.2cm}p{1.0cm}p{4.2cm}}
\toprule
...
\begin{lstlisting}[language=Python, caption={CSP backtracking with MRV + forward checking}]
...
\begin{lstlisting}[language=Python, caption={Production rule with multilingual keyword support}]
PRODUCTION_RULES = [
  {
    'id': 'R1_AnxietyStress',
