...
701 verses across 18 chapters, 16 classes, and 9 object properties—including
...
\texttt{paraphrase-multilingual-MiniLM-L12-v2} (50+ languages) over all 701
verses; and (7) an Ollama-powered local LLM generating commentary in English,
Hindi, and Hinglish.

We evaluate the retrieval pipeline against a 50-query gold set (10 semantic
categories, EN/Hindi/Hinglish). The full system achieves MAP@5 = 0.285 and
MRR = 0.637, outperforming BM25 by 10.2$\times$. Ablation confirms the
ontology contributes 81.6\% of retrieval quality; graph BFS contributes 31.9\%.
On Hindi-only queries, MRR = 0.800 vs.\ 0.067 for BM25 (12$\times$).
A* achieves 3-hop paths to \textit{Mok\d{s}a} with 63\% fewer expansions than
UCS; CSP satisfies all 7 constraints in under 0.3~s; and MYCIN reaches
...
\begin{itemize}[leftmargin=*]
  \item \textbf{OWL~2 Ontology:} 16 classes, 9 properties (\texttt{owl:TransitiveProperty},
        \texttt{owl:SymmetricProperty}, property chain), 3,523 RDF triples covering
        all 701 verses and 18 chapters; 13 SPARQL Competency Questions.

  \item \textbf{Multi-Algorithm Graph Search:} BFS, DFS, A*, IDDFS on a
        61-node, 1,657-edge concept graph; admissible A* heuristic;
        full UI pathway visualization.

  \item \textbf{Production-Rule Expert System:} 16-rule forward-chaining
        (R1--R16) with Hindi/Hinglish/Unicode keyword support, specificity-based
        conflict resolution; R11--R16 extend to loneliness, fear-of-death,
        purpose, jealousy, family conflict, failure.

  \item \textbf{CSP Study Planner:} Backtracking with MRV + forward checking,
        7 hard constraints, personalized verse scheduling.

  \item \textbf{Multi-Modal Uncertainty Handler:} MYCIN certainty factors,
        fuzzy yoga-path membership, non-monotonic belief revision.

  \item \textbf{Multilingual Semantic RAG:} \texttt{paraphrase-multilingual-MiniLM-L12-v2}
        (50+ languages) cosine search over all 701 verses; evaluated against
        a 50-query gold set: MAP@5 = 0.285, MRR = 0.637 (10.2$\times$ BM25).

  \item \textbf{Ollama LLM Integration:} Local Llama~3 commentary in English,
        Hindi, and Hinglish; no API key or cloud dependency.

  \item \textbf{React SPA + REST API:} 7 page components, ancient manuscript UI
        (React~18, Tailwind, Framer Motion), Flask REST API with 27 endpoints.
\end{itemize}
...
\begin{enumerate}[leftmargin=*]
  ...
  \item Embedding generation: All 701 verse English translations encoded with
        sentence-transformer \texttt{paraphrase-multilingual-MiniLM-L12-v2} and stored as a normalized NumPy matrix (\texttt{Data/embeddings/verse\_embeddings.npy}).
\end{enumerate}
...
Ontology       & RDF/OWL 2 (Turtle)       & W3C Rec.       & Embeddings        & sentence-transformers     & 3.0 \\
Semantic eng.  & RDFLib                   & 7.0            & Embedding model   & paraphrase-multilingual-MiniLM & 384-d \\
Query lang.    & SPARQL 1.1               & W3C Rec.       & Vector search     & NumPy cosine similarity    & 1.26 \\
...
Data layer        & Python csv module          & stdlib            & \multicolumn{3}{l}{\textit{Frontend (JavaScript)}} \\
               &                          &                & Framework         & React                     & 18 \\
...
\textit{krodha}, \textit{dhyan}), and Unicode Hindi (\textit{\hi{चिंता}},
\textit{\hi{गुस्सा}}, \textit{\hi{ध्यान}}). R11--R16 (v2.2) extend to
loneliness (\textit{akela}, \textit{\hi{अकेलापन}}) $\to$ Verse~6.5;
fear of death $\to$ Verse~2.20; purpose-seeking $\to$ Verse~3.35;
jealousy (\textit{jalan}) $\to$ Verse~2.47; family conflict $\to$ Verse~3.35;
failure (\textit{haar gaya}, \textit{\hi{निराश}}) $\to$ Verse~2.55.
...
\texttt{sentence-transformers/paraphrase-multilingual-MiniLM-L12-v2} (384-dimensional vectors).
...
sentence-transformers~3.0, NumPy~1.26, rank-bm25~0.2.2, Ollama~0.3, React~18. No GPU was
...
\begin{itemize}[leftmargin=*]
  ...
  \item \textit{Ontology completeness:} Fraction of 13 CQs answerable correctly.
...
for all KarmaYoga verses. CQ10 found the transitive path to Sam\={a}dhi.
CQ12 confirmed all four YogaPath instances \texttt{leadsTo} Mok\d{s}a.
...
\begin{table*}[!t]
\caption{Retrieval Performance Across System Variants (50 gold queries, $k$=5)}
\label{tab:retrieval}
\centering
...
\midrule
\multicolumn{6}{l}{\textit{Hindi-only subset (Q46--Q50):}} \\
BM25 (Hindi)      & 0.022 & 0.067 & 0.000 & --- & --- \\
Semantic (Hindi)  & 0.089 & 0.267 & 0.200 & --- & --- \\
Full System (Hindi)& 0.300 & \textbf{0.800} & \textbf{0.800} & --- & --- \\
\bottomrule
\end{tabular}
\end{table*}

Full system outperforms BM25 by \textbf{10.2$\times$} MAP@5 and
\textbf{10.9$\times$} MRR. Hindi-query MRR = 0.800 vs.\ 0.067 for BM25
(\textbf{12$\times$} improvement), validating the multilingual model swap.
``Feeling lost in duty'' correctly retrieves Verse~3.35 even though ``lost''
does not appear in the translation—true semantic matching.

\begin{table}[htbp]
\caption{Per-query Cosine Similarity (qualitative, not MAP@5)}
\label{tab:rag}
\centering
\small
\begin{tabular}{@{}p{3.2cm}lr@{}}
\toprule
\textbf{Query} & \textbf{Top-1} & \textbf{Sim.} \\
\midrule
``feeling lost in duty''       & 3.35 & 0.87 \\
``detach from outcomes''       & 2.47 & 0.91 \\
``restless mind meditation''   & 6.35 & 0.88 \\
``\hi{मुझे अकेलापन लगता है}'' & 6.5  & 0.79 \\
...
\textbf{Retrieval Ablation (see Table~\ref{tab:retrieval}):} Removing the ontology
drops MAP@5 by 81.6\% (dominant component). Removing graph BFS drops 31.9\%.
Expert rules contribute 8.2\%; semantic RAG adds 2.8\% marginally but enables
Hindi queries (MRR 0.800). Component interactions are super-additive.
...
Evaluated against a 50-query gold set: MAP@5 = 0.285, MRR = 0.637 (10.2$\times$
over BM25); Hindi-query MRR = 0.800 (12$\times$ over BM25). Ablation confirms
the ontology is the dominant retrieval component (81.6\% contribution). The
ontology covers all 701 verses (3,523 RDF triples); the expert system has 16
rules and 13 validated CQs. A* finds 3-hop paths with 63\% fewer expansions;
CSP satisfies all 7 constraints with 93.6\% fewer backtracks; MYCIN reaches
$CF = 0.9991$. All modules are deployed in a React SPA with a 27-endpoint
Flask REST API.
...
Hariom Rajput \newline (524110006)
  & Expert System
  & Module 4: 16-rule forward-chaining engine (R1--R16); Hindi/Hinglish/Unicode keyword support; specificity-ordered conflict resolution; 13 SPARQL CQs (CQ1--CQ12 + CQ\_CONSTRUCT); R11--R16 for loneliness, fear-of-death, purpose, jealousy, family conflict, failure. \\
\midrule
Ayushi Choyal \newline (524410017)
  & Uncertainty
...
Ravi Kant Gupta \newline (524410027)
  & Team Lead
  & Module 1 (PEAS); Module 2 (OWL~2 ontology, 3,523 triples, all 701 verses, 18 chapters, rule-based expansion); Module 7 (Multilingual RAG, \texttt{paraphrase-multilingual-MiniLM-L12-v2}; Ollama LLM; EN/Hindi/Hinglish); Evaluation framework (50-query gold set, MAP@5/MRR, 7-variant ablation); Flask REST API (27 endpoints); React SPA architecture. \\
\midrule
Shouryavi Awasthi \newline (524410028)
  & Graph \& CSP
